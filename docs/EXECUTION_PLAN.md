...
## Status (2026-09-20 ~03:55Z) — gate clean (87/87, 206/206 claims) after the week-long suspension; discovery and C2 1,000-sim runs resumed; cold passes relaunched
...
| Gate | **88 checks (X.run_all_deterministic_env added 04:15Z), 87/87 PASS twice at 03:50Z 2026-09-20 before it, baseline refreshed; 206 of 206 claims verify.** The eight §8 randomization p-values moved to their 2,000-draw values (0.695→0.697, 0.519→0.527, 0.880→0.886, 0.826→0.822; the updater moved them, the prose says 2,000 draws, C167/C172 re-anchored). Checks added 2026-09-13: S.lift_requires_1000_sims, X.run_all_refresh_guard, S.confirmatory_spec_audit, S.calibration_noise_measured, S.confirmatory_reading_rules, V.table1_regenerates, D.discovery_scripts_pinned. `S.ri_scheme_certified` PASS since 20:56Z (discovery anchor_shift@200, C1 circular_within_block_fw7@1000, C2 anchor_shift@200, pooled circular_within_block_fw7@200); `S.lift_requires_1000_sims` PASS with discovery and C2 still short, the 1,000-sim runs in flight. Run twice at 20:57Z, identical: 73 PASS, 8 BLOCKED, 2 FAIL — every non-PASS is artifact-pending because the cold run's clear (20:55:49Z) had just deleted `data/processed` and the model tables: the 8 BLOCKED name the missing panel/CAI inputs, `V.claims_reproduce` reports 89 of 206 absent, and `M.status_honest` follows those BLOCKED checks (a BLOCKED check is not evidence). The gate is re-run after cold pass 1 rebuilds them. `main()` reports an unbaselined check as a failing condition; the baseline covers every check. |
| Findings | 125: 125 fixed, 0 open, 0 unverified (N11 21:08Z, D8 21:40Z, P19–P26 from the second specification audit 22:20Z, X20 2026-09-20; all fixed, the checks defeat-tested). P1, P5, RI3, P12 and P13 closed at 20:56Z on C1's 1,000-simulation certificate (they had closed at 12:20Z on the old certificates and reopened at 17:30Z when P13 was measured). The CP2 audit added P13–P18, O6, F4, X18 and X19. **C1 certified 2026-09-13 20:56Z** under `circular_within_block_fw7`, 1,000 × 200: rejection 0.049 (band [0.0365, 0.0635]), KS D 0.0155, lattice p 0.9930, median p 0.4975, CALIBRATED — after the same drawer's 200-sim run had failed uniformity (D 0.097, p 0.0455). Addendum §25, written between the two verdicts, is therefore **not invoked**; it stays in the record as the pre-registered fallback it was. |
