...
- **The run, 09:29:42Z → 03:24:13Z.** After the second OOM kill (20:17Z, a
  ...
  Every one of the seven starts is a run-log row with its reason (suspension,
  automatic retry after OOM, hand relaunch with fewer workers, three workers for
  speed, the second OOM and its relaunch); the sealed table's `overwrite_reason`
  column carries the last reason only, which §8b and Methods say.
