...
- **2026-09-21 03:40Z** — 2026-09-21 (assistant) — Phase I sealed 03:24:13Z after seven starts (suspension, two OOM kills, worker-count changes; every reason in the run log, the last in overwrite_reason). Reading: the pre-specified null in both strata; MDE 0.01156/0.00875 disfavoured, MEI -0.005 not excluded, transient dip disfavoured, placebos quiet; C1's 14/24 sensitivity cells and positive dose-response reported, not read. §8b, PAPER.md Results/Discussion/Abstract written from the reading only; 1,269 claims verify; gate 90/90 twice. Two post-paste fixes: table labels may not begin with a numeral; the C161 anchor gained its citation marker.

- **2026-09-21 05:27Z** — 2026-09-21 (assistant) — CP3 full manuscript audit (wf_0df7b965: 8 finders, 61 raw/57 unique, 3-lens refutation of the ten highest -> 5 survived, critic 6) remediated in full at 663153d; filed P59-P63 with defeat-tested suite tokens. The paper's account of the record now uses the record's nouns (F1-F3 read outcome values; F2 decision and F4 weights kept; commitments postdate F1-F3; damping runs in the predicted direction); disposition filter, H2/H3 limitation, nine pooled cells, captions, Figure 2 units, Figure 3 joint spec, Wu et al., family-corrected bound. 1,354 claims verify; 164 findings fixed; gate 90/90. Manuscript complete; the author's editorial pass (word and display-item budget) is the open item.

- **2026-09-21 17:49Z** — 2026-09-21 (assistant) — Vault reorganised for Obsidian: hub 00 Project with a map of content; new topic notes Record, Freeze incidents, Phase I, Manuscript, Gate and checks, Audits, Lessons, Timeline; existing notes given frontmatter and Related footers; the memory graph rendered as vault/Graph/<id>.md (one note per node, edges as wikilinks, result values redacted) by ops/graph_memory/obsidian.py with index Graph memory.md. Every wikilink resolves; no orphan notes.

- **2026-09-21 22:51Z** — 2026-09-21 (assistant, under the user's standing mandate) — The manuscript restructured to the venue's budget (03e3d8e: docs/SUPPLEMENT.md, prose Results, four display items, V.paper_budget) and then revised on a five-referee panel with a fact-check per major comment (d999d84): intervals (mean ± 1.96 SE) are reported beside the verbatim pre-registered reading, which stands as the conclusion; the C1 path, the two guards, the count arm and the diagnostics are described as the sealed files have them (RP1–RP6, V.manuscript_referee_tokens); the within-period family of four is disclosed with the period-level reading unchanged. Nothing in the specification, the family, the reading rules or the reading changed. Open for the author: the venue's ~4,000 words, whether the two additions to the pre-registered read-out stay, the display-item choice, Figure S7 and a counted linkage flow (need artifacts), ethics and front matter, RECORD 6.2. — docs/REFEREE_PANEL_2026-09-21.md, EXECUTION_PLAN Status 22:50Z.
