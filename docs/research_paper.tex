...
Table 4 presents results from the difference-in-differences analysis using high awareness days (awareness_z > 1.5 SD) as treatment:

\begin{table}[H]
\centering
\caption{Difference-in-Differences: Effect of High Awareness Days}
\label{tab:did}
\begin{tabular}{lcc}
\toprule
 & Coefficient & Standard Error \\
\midrule
Simple 2×2 DID & 0.00205 & (clustered) \\
\midrule
Observations & \multicolumn{2}{c}{13,693} \\
R-squared & \multicolumn{2}{c}{0.0985} \\
\bottomrule
\end{tabular}
\footnotesize
Note: Treatment is defined as days with awareness_z > 1.5 SD. The DID coefficient represents the difference in mental health call share between treated and control days, after accounting for pre/post differences. The positive coefficient is consistent with the distributed lag and threshold findings.
\end{table}

The DID analysis confirms the main finding: high awareness days lead to increased mental health call share. The event study specification (not shown) reveals positive effects in the days following high awareness events, with no clear pre-trends, supporting the causal interpretation.
...
\textbf{Multiple Approaches Confirm Findings:} The consistency of findings across distributed lag models, threshold analysis, and difference-in-differences approaches strengthens confidence in the results and suggests the effect is robust to different modeling assumptions.
